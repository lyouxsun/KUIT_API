...
public class UserService {
    ...
    public PostUserResponse signUp(PostUserRequest postUserRequest) {
        ...
        // TODO: 4. JWT 토큰 생성
        String jwt = jwtTokenProvider.createToken(postUserRequest.getEmail(), userId);  //토큰을 생성하고, response로 넣어주는 로직
    ...
    public void modifyUserStatus_dormant(long userId) {
        ...
        long affectedRows = userDao.modifyUserStatus_dormant(userId);
        if (affectedRows != 1) {
    ...
    public void modifyUserStatus_deleted(long userId) {
        ...
        long affectedRows = userDao.modifyUserStatus_deleted(userId);
        if (affectedRows != 1) {
            throw new DatabaseException(DATABASE_ERROR);
        }
    }
    ...
    public void modifyNickname(long userId, String nickname) {
        ...
        long affectedRows = userDao.modifyNickname(userId, nickname);
        if (affectedRows != 1) {
            throw new DatabaseException(DATABASE_ERROR);
        }
    }
    ...
    public void modifyPassword(long userId, String password) {
        ...
        validateNickname(password);
        long affectedRows = userDao.modifyPassword(userId, password);
        if (affectedRows != 1) {
            throw new DatabaseException(DATABASE_ERROR);
        }
    }
    ...
    public void postAddress(long userId, String address) {
        log.info("[UserService.postAddress]");
